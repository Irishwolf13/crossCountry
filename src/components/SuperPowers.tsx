...
const SuperPowers: React.FC = () => {
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        const { questions, videoURL } = await fetchQuestionsAndVideoByName("questionares", "superPowers");
  ...
  const handleSave = async () => {
    try {
      await updateAnswers("questionares", "superPowers", editedItems);
  ...
  const handleAddQuestion = async () => {
    if (newQuestion.trim() !== '') {
      try {
        await addQuestionToDocument("questionares", "superPowers", newQuestion);
  ...
  const handleVideoUpload = async (file: File) => {
    try {
      ...
      if (result) {
        ...
        await updateVideoURL("questionares", "superPowers", downloadURL);

...
const ComedyMovies: React.FC = () => {
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        const { questions, videoURL } = await fetchQuestionsAndVideoByName("questionares", "comedyMovies");
  ...
  const handleSave = async () => {
    try {
      await updateAnswers("questionares", "comedyMovies", editedItems);
  ...
  const handleAddQuestion = async () => {
    if (newQuestion.trim() !== '') {
      try {
        await addQuestionToDocument("questionares", "comedyMovies", newQuestion);
  ...
  const handleVideoUpload = async (file: File) => {
    try {
      ...
      if (result) {
        ...
        await updateVideoURL("questionares", "comedyMovies", downloadURL);

...
const Colbert: React.FC = () => {
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        const { questions, videoURL } = await fetchQuestionsAndVideoByName("questionares", "colbert");
  ...
  const handleSave = async () => {
    try {
      await updateAnswers("questionares", "colbert", editedItems);
  ...
  const handleAddQuestion = async () => {
    if (newQuestion.trim() !== '') {
      try {
        await addQuestionToDocument("questionares", "colbert", newQuestion);
  ...
  const handleVideoUpload = async (file: File) => {
    try {
      ...
      if (result) {
        ...
        await updateVideoURL("questionares", "colbert", downloadURL);
